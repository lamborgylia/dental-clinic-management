#!/usr/bin/env python3
"""
Скрипт для создания записей на текущую неделю
"""
...
def main():
    print("🚀 Создаем записи на текущую неделю...")
    ...
    # Создаем записи для врача с ID=6 (Федор) - у него уже есть 100 записей на эту неделю
    doctor_id = 6
    ...
    # Создаем по 30-40 записей в день для врача 6
    current_day = start_of_week
    while current_day <= end_of_week:
        print(f"\n📅 Создаем записи на {current_day.strftime('%d.%m.%Y')}...")
        
        # Создаем 30-40 записей в день для врача 6
        num_appointments = random.randint(30, 40)
